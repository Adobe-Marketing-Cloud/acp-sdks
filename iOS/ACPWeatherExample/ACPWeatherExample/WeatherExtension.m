...
+ (void) registerExtension {
    ...
    if ([ACPCore registerExtension:[WeatherExtensionInternal class] error:&error]) {
        [ACPCore log:ACPMobileLogLevelDebug tag:@"WeatherExtension" message:@"WeatherExtension was successfully registered"];
    } else {
        [ACPCore log:ACPMobileLogLevelError tag:@"WeatherExtension" message:[NSString stringWithFormat:@"An error occurred while attempting to register WeatherExtension: %@", [error localizedDescription]]];
    }
}
...
+ (void) getWeatherByZipCode:(NSUInteger)zip callback:(nullable void (^) (WeatherExtensionDataObject* __nullable content))callback {
    ...
    ACPExtensionEvent *getWeatherEvent = [ACPExtensionEvent extensionEventWithName:@"Get Weather"
                                                                            ...
                                                                            source:@"com.acpExample.eventSource.requestContent"
                                                                             ...
                                                                             error:&eventError];
    if (!getWeatherEvent) {
        [ACPCore log:ACPMobileLogLevelError tag:@"WeatherExtension" message:[NSString stringWithFormat:@"An error occurred constructing event '%@': %@", getWeatherEvent.eventName, eventError.localizedDescription ?: @"unknown"]];
    }
    ...
    if ([ACPCore dispatchEventWithResponseCallback:getWeatherEvent responseCallback:^(ACPExtensionEvent * _Nonnull responseEvent) {
        [ACPCore log:ACPMobileLogLevelDebug tag:@"WeatherExtension" message:[NSString stringWithFormat:@"In response callback for event '%@', event data returned: %@", responseEvent.eventName, responseEvent.eventData]];
        ...
        [ACPCore log:ACPMobileLogLevelDebug tag:@"WeatherExtension" message:[NSString stringWithFormat:@"WeatherExtension dispatched an event '%@'", getWeatherEvent.eventName]];
    } else {
        [ACPCore log:ACPMobileLogLevelError tag:@"WeatherExtension" message:[NSString stringWithFormat:@"An error occurred dispatching event '%@': %@", getWeatherEvent.eventName, dispatchError.localizedDescription ?: @"unknown"]];
    }
}
