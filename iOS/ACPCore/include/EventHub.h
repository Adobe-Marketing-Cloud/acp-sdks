...
namespace AdobeMarketingMobile {
    ...
    public:
         ...
         * @param platform_services_factory_function - a factory function to provide a new instance of PlatformServices to each extension
         */
        ...
        template<class TModule>
        void RegisterOneTimeListener(const std::string& pair_id,
                                     const std::function<void(const std::shared_ptr<Event>&)>& listener_block);
    ...
    private:
         ...
         * @private
         *
         * Dispatch an event and share state at the same time. shared state will have the
         * outgoing event's event number.
         * @param module  Module that owns this shared state
         * @param state  EventData object containing the state to share.
         * @param event - the event to be added to the queue
         */
        virtual void CreateSharedStateAndDispatchEvent(const std::shared_ptr<Module>& module,
                const std::shared_ptr<EventData>& state, const std::shared_ptr<Event>& event) = 0;

        /**
         * @private
         *
         * Replace the rules for the specific module with a new set of rules. This API is to replace the RegisterRule, which can only be used to
         * add rules one by one.
         */
        virtual void ReplaceRulesSet(const std::shared_ptr<Module>& module,
                                     const std::vector<std::shared_ptr<Rule>> rules_vector) = 0;
    };
}
